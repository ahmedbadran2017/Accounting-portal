<template>
  <div v-if="err" class="bg-white border border-line rounded-card shadow-card p-8 text-center max-w-lg mx-auto mt-10">
    ...
    <div class="text-[16px] font-bold mt-3">{{ L("This page hit an error", "حصل خطأ في الصفحة", "Erreur sur cette page") }}</div>
    ...
    <div class="flex items-center justify-center gap-2 mt-4">
      <UiButton variant="primary" size="md" @click="retry">{{ L("Reload page", "إعادة التحميل", "Recharger") }}</UiButton>
      <UiButton variant="secondary" size="md" @click="resetFilters">{{ L("Reset saved filters", "إعادة ضبط الفلاتر", "Réinitialiser les filtres") }}</UiButton>
    </div>
  </div>

<template>
  <div class="space-y-3.5">
    ...
    <div class="flex items-center gap-1.5 flex-wrap">
      <button v-for="k in KINDS" :key="k.key" @click="setKind(k.key)" class="text-[12px] font-semibold px-3 py-1.5 rounded-full border transition inline-flex items-center gap-1.5"
              :class="kind === k.key ? 'bg-ink text-white border-ink' : 'bg-white text-ink-3 border-line-2 hover:bg-app-warm'">
        {{ k.label() }}<span v-if="counts[k.key] != null" class="text-[11px] font-bold px-1.5 py-0.5 rounded-full" :class="kind === k.key ? 'bg-white/20' : 'bg-app-warm text-ink-3'">{{ counts[k.key] }}</span>
      </button>
    ...
    <div class="bg-white rounded-card border border-line overflow-hidden shadow-card">
      ...
      <div v-else-if="!rows.length" class="py-14 text-center"><Icon name="check" :size="22" color="#047857" /><div class="text-[13px] text-success-dark font-semibold mt-1">{{ L("All caught up — every document here has its source file.","تمام — كل المستندات هنا مرفوعة.","Tout est à jour.") }}</div></div>
        ...
        <table class="w-full text-[12px]">
          ...
          <tbody>
            <tr v-for="r in rows" :key="r.name" class="border-t border-line-hair hover:bg-app-warm/40">
              <td class="px-4 py-2.5 font-mono text-[12px] font-semibold cursor-pointer hover:text-accent-dark" @click="openDoc(r.name)">{{ r.name }}</td>

  ...
  <div class="space-y-3.5">
    <div class="flex items-center gap-2 flex-wrap">
      ...
      <div class="ms-auto flex items-center gap-1.5">
        <button @click="view = 'month'" class="text-[12px] font-semibold px-3 py-1.5 rounded-full border transition"
        ...
        <button @click="view = 'cycle'; if (!cy) loadCycle();" class="text-[12px] font-semibold px-3 py-1.5 rounded-full border transition"
        ...
        <span class="w-2"></span>
        <button v-for="yy in years" :key="yy" @click="year = yy; reload()" class="text-[12px] font-semibold px-3 py-1.5 rounded-full border transition"
      ...
      <template v-else-if="cy">
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div class="bg-white rounded-card border border-line shadow-card px-4 py-3">
            <div class="text-[11px] font-bold uppercase tracking-wider text-ink-muted">{{ L("Orders","أوردرات","Commandes") }}</div>
            <div class="text-[20px] font-extrabold tnum" dir="ltr">{{ n(cy.orders) }}</div>
          </div>
          <div class="bg-white rounded-card border border-line shadow-card px-4 py-3">
            <div class="text-[11px] font-bold uppercase tracking-wider" style="color:#047857">{{ L("Complete story","قصة كاملة","Histoire complète") }}</div>
            <div class="text-[20px] font-extrabold tnum" style="color:#047857" dir="ltr">{{ n(cy.complete) }} <span class="text-[12px]">({{ cy.orders ? Math.round(100*cy.complete/cy.orders) : 0 }}%)</span></div>
          </div>
          <div class="bg-white rounded-card border border-line shadow-card px-4 py-3">
            <div class="text-[11px] font-bold uppercase tracking-wider" style="color:#b91c1c">{{ L("Incomplete","قصة ناقصة","Incomplète") }}</div>
            <div class="text-[20px] font-extrabold tnum" style="color:#b91c1c" dir="ltr">{{ n(cy.incomplete) }}</div>
          </div>
          <div class="bg-white rounded-card border border-line shadow-card px-4 py-3">
            <div class="text-[11px] font-bold uppercase tracking-wider text-ink-muted">{{ L("The contract","العقد","Le contrat") }}</div>
            <div class="text-[11px] text-ink-3 leading-snug mt-0.5">{{ L("delivered→DN+SI · returned pre-invoice→no SI · post-invoice→CN+refund","متسلم→إذن+فاتورة · راجع قبل الفوترة→بلا فاتورة · بعدها→CN+استرداد","livré→BL+facture · retour→avoir+remb.") }}</div>
          </div>
        </div>
        <div class="bg-white rounded-card border border-line shadow-card overflow-hidden">
          <table class="w-full text-[12px]">
            ...
            <tbody>
              ...
              <tr v-if="!Object.keys(cy.buckets || {}).length"><td colspan="4" class="px-4 py-10 text-center text-[13px]" style="color:#047857">{{ L("Every order tells its complete story. 🎉","كل أوردر بيحكي قصته كاملة 🎉","Toutes les commandes sont complètes 🎉") }}</td></tr>
            </tbody>
          </table>
        </div>
        <div v-if="cyBucket" class="bg-white rounded-card border border-line shadow-card overflow-hidden">
          <div class="px-4 py-2.5 text-[12px] font-bold border-b border-line-hair" style="background:#fafaf9">{{ cyLabel(cyBucket) }} <span class="text-ink-muted font-normal" dir="ltr">{{ drillRows.length }} / {{ drillTotal }}</span></div>
            ...
            <tbody>
              <tr v-for="d in drillRows" :key="d.so" class="border-t border-line-hair first:border-t-0 hover:bg-app-warm/30">
                ...
                <td class="px-4 py-2 text-end w-32">
                  <UiButton variant="secondary" size="xs" v-if="d.action === 'bill'" :disabled="busy === d.doc" @click="bill(d)">{{ busy === d.doc ? "…" : L("Make invoice","اعمل فاتورة","Facturer") }}</UiButton>
                  <UiButton variant="danger" size="xs" v-else-if="d.action === 'credit_note'" :disabled="busy === d.doc" @click="creditNote(d)">{{ busy === d.doc ? "…" : L("Credit note","إشعار دائن","Avoir") }}</UiButton>
    ...
    <div v-if="view === 'month'" class="bg-white rounded-card border border-line overflow-hidden shadow-card">
      ...
      <div v-else-if="!rows.length" class="py-14 text-center text-[13px] text-ink-muted">{{ L("No data for this year.","لا توجد بيانات لهذه السنة.","Aucune donnée.") }}</div>
            ...
            <template v-for="r in rows" :key="r.month">
              ...
              <tr v-if="open === r.month" class="border-t border-line-hair" style="background:#fafaf9">
                <td colspan="9" class="px-4 py-3">
                  ...
                  <table v-else-if="drillRows.length" class="w-full text-[12px] bg-white rounded-[10px] border border-line-hair">
                    <tbody>
                      <tr v-for="d in drillRows" :key="d.doc" class="border-t border-line-hair first:border-t-0">
                        ...
                        <td class="px-3 py-2 text-end w-32">
                          <UiButton variant="secondary" size="xs" v-if="d.action === 'bill'" :disabled="busy === d.doc" @click="bill(d)">
                            {{ busy === d.doc ? "…" : L("Make invoice","اعمل فاتورة","Facturer") }}
                          </UiButton>
                          <UiButton variant="danger" size="xs" v-else-if="d.action === 'credit_note'" :disabled="busy === d.doc" @click="creditNote(d)">
                  ...
                  <div v-else class="text-[12px] text-ink-muted py-2">{{ L("Pick a bucket above to list its documents.","اختر فئة من فوق لعرض مستنداتها.","Choisissez une catégorie.") }}</div>
                  <div v-if="drillTotal > drillRows.length" class="text-[11px] text-ink-muted mt-1.5" dir="ltr">{{ drillRows.length }} / {{ drillTotal }}</div>
                </td>
              </tr>
            </template>
          </tbody>
    ...
    <div v-if="view === 'month' && d0 && d0.tie_out" class="bg-white rounded-card border border-line shadow-card px-4 py-3">
      <div class="text-[12px] font-bold mb-1.5">{{ L("Ties out to the P&L COGS section","التسوية مع بند الكوجز في قائمة الدخل","Rapproché du P&L") }}</div>
      <div class="flex items-center gap-x-4 gap-y-1 flex-wrap text-[12px] tnum" dir="ltr">
      ...
      <div class="text-[11px] text-ink-muted mt-1">{{ L("Direct bills/receipts/JEs in COGS are pollution being cleaned — this line shrinking to 'DN cost = GL' is the goal.","الفواتير والقيود المباشرة في الكوجز تلوث بيتنضف — الهدف إن السطر ده يبقى: تكلفة الأذون = الدفاتر.","Les écritures directes sont en cours de nettoyage.") }}</div>
    </div>
  </div>
</template>

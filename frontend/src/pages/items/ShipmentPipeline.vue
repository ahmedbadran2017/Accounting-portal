          ...
          <tr v-for="d0 in drafts" :key="d0.name" class="border-t border-line-hair align-top">
            ...
            <td class="px-4 py-2.5 text-end whitespace-nowrap">
              <div v-if="canWrite" class="inline-flex items-center gap-1.5">
                <UiButton variant="secondary" size="xs" type="button" :disabled="busy===d0.name" @click="submitDraft(d0)">{{ busy===d0.name ? '…' : L('Post','رحّل','Valider') }}</UiButton>
                <UiButton variant="danger" size="xs" type="button" :disabled="busy===d0.name" @click="dropDraft(d0)">{{ L("Delete","احذف","Suppr.") }}</UiButton>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <div class="bg-white rounded-card border border-line shadow-card overflow-hidden">
      <div class="px-4 py-2.5 border-b border-line-hair text-[12px] font-bold flex items-center gap-2 flex-wrap">
        ...
        <div class="ms-auto inline-flex items-center gap-2">
          <UiButton variant="secondary" size="sm" type="button" :disabled="!canPreview || previewing" @click="doPreview">{{ previewing ? '…' : L("Preview","عاين","Aperçu") }}</UiButton>
          <UiButton variant="primary" size="sm" type="button" :disabled="!preview || posting || !canWrite || preview.blocked" @click="doPost">{{ posting ? '…' : L("Post landed cost","رحّل التحميل","Valider") }}</UiButton>
        </div>
      </div>

<template>
  <div class="space-y-3">
    ...
    <div v-if="pending" class="flex items-center gap-2.5 px-4 py-2.5 rounded-card border" style="background:#fffbeb;border-color:#fde68a">
      ...
      <UiButton variant="secondary" size="xs" class="ms-auto" @click="filter = 'Proposed'" >{{ L("Review", "راجع", "Examiner") }}</UiButton>
    </div>
          ...
          <tr v-for="a in filtered" :key="a.name" class="border-t border-line-hair hover:bg-app-warm/40">
            ...
            <td class="px-4 py-2.5 text-end whitespace-nowrap">
              <span class="inline-flex gap-1.5 items-center">
                <UiButton variant="secondary" size="xs" icon="doc" @click="openDetail(a)" :title="L('View transaction details','عرض تفاصيل المعاملة','Voir les détails')"> {{ L("Details","تفاصيل","Détails") }}</UiButton>
                <span v-if="a.status === 'Proposed'" class="relative">
                  <UiButton variant="secondary" size="xs" icon="user" @click="assignOpen = assignOpen === a.name ? '' : a.name" > {{ L("Assign","إسناد","Assigner") }}</UiButton>
                ...
                <UiButton variant="secondary" size="xs" v-if="a.status === 'Proposed' && !isMine(a)" @click="approve(a)" :disabled="busy" >{{ L("Approve", "اعتماد", "Approuver") }}</UiButton>
                <UiButton variant="secondary" size="xs" v-else-if="a.status === 'Proposed' && canBreakGlass" @click="selfApprove(a)" :disabled="busy" :title="L('No other approver available — self-approve with a reason (logged)','لا يوجد موافِق آخر — اعتمد بنفسك بسبب مُسجّل','Auto-approuver')">{{ L("Self-approve", "اعتمد بنفسك", "Auto-approuver") }}</UiButton>
                ...
                <UiButton variant="danger" size="xs" v-if="a.status === 'Proposed'" @click="reject(a)" :disabled="busy" >{{ L("Reject", "رفض", "Rejeter") }}</UiButton>
                <UiButton variant="secondary" size="xs" v-if="a.status === 'Posted' && a.revertable && canUndo" @click="revert(a)" :disabled="busy" ><Icon name="arrow" :size="11" class="rotate-180" />{{ L("Undo", "تراجع", "Annuler") }}</UiButton>
              </span>
            </td>
          </tr>
    ...
    <div v-if="detailOpen" class="fixed inset-0 z-40 flex items-center justify-center p-4" @click.self="closeDetail">
      ...
      <div class="relative bg-white rounded-[16px] shadow-pop w-full max-w-lg max-h-[86vh] flex flex-col overflow-hidden">
        ...
        <footer v-if="detail && detail.status === 'Proposed'" class="flex items-center gap-2 px-4 py-3 border-t border-line-hair bg-app-warm/30">
          <UiButton variant="danger" size="sm" @click="reject(detail)" :disabled="busy" >{{ L("Reject","رفض","Rejeter") }}</UiButton>
          <UiButton variant="primary" size="sm" class="ms-auto" v-if="!isMine(detail)" @click="approve(detail)" :disabled="busy" >{{ L("Approve & post","اعتماد وترحيل","Approuver") }}</UiButton>
          <UiButton variant="secondary" size="sm" class="ms-auto" v-else-if="canBreakGlass" @click="selfApprove(detail)" :disabled="busy" >{{ L("Self-approve","اعتمد بنفسك","Auto-approuver") }}</UiButton>

  ...
  <div class="min-h-screen grid place-items-center bg-app-bg p-4">
    <div class="w-full max-w-md bg-white rounded-card shadow-card border border-line p-7">
      <div class="flex items-center gap-3 mb-5">
        ...
        <div>
          <div class="text-[17px] font-bold text-ink">{{ L("The Desk is closed for your account", "الـ Desk مقفول على حسابك", "Le Desk est fermé pour votre compte") }}</div>
          <div class="text-[11.5px] text-ink-muted">{{ me || "…" }}</div>
        </div>
      </div>
      ...
      <template v-else-if="!st.locked">
        <p class="text-[12.5px] text-ink-2 mb-4">{{ L("Your account is not locked. You can open the Desk directly.", "حسابك مش مقفول. تقدر تفتح الـ Desk مباشرة.", "Votre compte n'est pas verrouillé.") }}</p>
      ...
      <template v-else-if="st.remaining_min > 0">
        <div class="rounded-[10px] px-3 py-2.5 mb-4 text-[12.5px]" style="background:#ecfdf5;color:#065f46">
      ...
      <template v-else>
        <p class="text-[12.5px] text-ink-2 leading-relaxed mb-3">
      ...
      <div class="mt-5 pt-4 border-t border-line-hair flex items-center justify-between text-[11.5px]">

    ...
    <template v-else-if="d">
        ...
        <div class="flex items-start gap-3 flex-wrap">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
              <span class="text-[16px] font-bold font-mono">{{ d.ref }}</span>
              <span class="text-[11px] font-bold px-2 py-0.5 rounded-full" :style="stBadge(d.status)">{{ stLabel(d.status) }}</span>
            </div>
            <div class="text-[12px] text-ink-muted mt-0.5">{{ d.carrier }} · {{ d.n_orders }} {{ L("orders","طلب","cmd") }} · {{ d.n_deposits }} {{ L("deposits","إيداع","dépôts") }}</div>
          </div>
        </div>
        <div class="grid grid-cols-3 gap-2 mt-4">
          <div class="rounded-[10px] px-3 py-2.5" style="background:#fafaf9;border:1px solid #f0efed">
            <div class="text-[11px] text-ink-muted font-bold uppercase tracking-wide">{{ L("Expected","المتوقَّع","Attendu") }}</div>
            <div class="text-[16px] font-bold tnum">{{ fmt(d.expected) }}</div>
          </div>
          <div class="rounded-[10px] px-3 py-2.5" style="background:#fafaf9;border:1px solid #f0efed">
            <div class="text-[11px] text-ink-muted font-bold uppercase tracking-wide">{{ L("Collected","المُحصَّل","Collecté") }}</div>
          ...
          <div class="rounded-[10px] px-3 py-2.5" :style="d.variance ? 'background:#fff5f5;border:1px solid #fecaca' : 'background:#f0fdf4;border:1px solid #bbf7d0'">
            <div class="text-[11px] text-ink-muted font-bold uppercase tracking-wide">{{ L("Variance","الفرق","Écart") }}</div>
      ...
      <div class="grid lg:grid-cols-2 gap-3.5">
        ...
        <div class="bg-white rounded-card border border-line shadow-card overflow-hidden">
          <div class="px-4 py-2.5 border-b border-line-hair flex items-center gap-2"><Icon name="cart" :size="14" color="#0b5c4f" /><span class="text-[12px] font-bold">{{ L("Orders","الطلبات","Commandes") }}</span><span class="text-[11px] text-ink-muted">{{ d.n_orders }}</span></div>
            ...
            <table class="w-full text-[12px]">
              <tbody>
                <tr v-for="(o, i) in d.orders" :key="i" class="border-t border-line-hair hover:bg-app-warm/50 cursor-pointer" @click="goOrder(o.name)">
                  <td class="px-4 py-2 font-mono text-[11px]">{{ o.name }}<div class="text-[11px] text-ink-muted font-sans truncate max-w-[150px]">{{ o.customer }}</div></td>
        ...
        <!-- Deposits -->
        <div class="bg-white rounded-card border border-line shadow-card overflow-hidden">
          <div class="px-4 py-2.5 border-b border-line-hair flex items-center gap-2"><Icon name="coins" :size="14" color="#047857" /><span class="text-[12px] font-bold">{{ L("Deposits","الإيداعات","Dépôts") }}</span><span class="text-[11px] text-ink-muted">{{ d.n_deposits }}</span></div>
          <div class="overflow-y-auto max-h-[360px]">
            <table class="w-full text-[12px]">
              <tbody>
                <tr v-for="(p, i) in d.deposits" :key="i" class="border-t border-line-hair hover:bg-app-warm/50 cursor-pointer" @click="goReceipt(p.name)">
                  <td class="px-4 py-2 font-mono text-[11px]">{{ p.name }}<div class="text-[11px] text-ink-muted font-sans">{{ p.date }} · {{ p.method }}</div></td>

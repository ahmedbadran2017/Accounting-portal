  ...
  <div class="space-y-3.5">
    <div class="grid grid-cols-2 lg:grid-cols-3 gap-3">
      <div class="bg-white rounded-card border shadow-card px-4 py-3" :class="d.dead_active ? 'border-amber-200' : 'border-line'">
        <div class="text-[11px] font-bold uppercase tracking-wider flex items-center gap-1.5" :class="d.dead_active ? 'text-amber-700' : 'text-ink-muted'"><Icon name="alert" :size="13" :color="d.dead_active ? '#b45309' : '#94a3b8'" />{{ L("Unused accounts","حسابات غير مستخدمة","Comptes inutilisés") }}</div>
        <div class="text-[20px] font-extrabold mt-1 tnum" :class="d.dead_active ? 'text-amber-700' : ''">{{ loading ? "—" : (d.dead_active || 0) }}</div>
        <div class="text-[11px] text-ink-muted mt-0.5">{{ L("of","من","sur") }} {{ (d.leaves || 0) }} · {{ L("no ledger activity","بلا حركة","aucune écriture") }}</div>
      </div>
      <div class="bg-white rounded-card border border-line shadow-card px-4 py-3">
        <div class="text-[11px] font-bold uppercase tracking-wider text-ink-muted flex items-center gap-1.5"><Icon name="layers" :size="13" color="#0369a1" />{{ L("Same-name pairs","أسماء مكرّرة","Noms dupliqués") }}</div>
        <div class="text-[20px] font-extrabold mt-1 tnum">{{ loading ? "—" : (d.name_pair_count || 0) }}</div>
        <div class="text-[11px] text-ink-muted mt-0.5">{{ L("review — often AR/AP pairs","راجع — غالباً مدين/دائن","souvent AR/AP") }}</div>
      </div>
        ...
        <table class="w-full text-[12px]">
          <tbody>
            <tr v-for="a in filtered" :key="a.account" class="border-t border-line-hair first:border-t-0 hover:bg-app-warm/40">
              ...
              <td class="px-3 py-2 text-ink-muted text-[11px]">{{ a.root }}</td>
    ...
    <div v-if="(d.name_pairs||[]).length" class="bg-white rounded-card border border-line shadow-card overflow-hidden">
      ...
      <div class="max-h-72 overflow-auto">
        <div v-for="(p,i) in d.name_pairs" :key="i" class="border-t border-line-hair first:border-t-0 px-4 py-2 text-[12px]">
          <div class="font-semibold truncate">{{ p.name }}</div>
          <div class="text-[11px] text-ink-muted flex flex-wrap gap-x-3">

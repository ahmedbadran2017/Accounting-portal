<template>
  <div class="space-y-3.5">
    <div class="flex items-center gap-2 flex-wrap">
      ...
      <button v-if="canWrite" type="button" class="ms-auto inline-flex items-center gap-1.5 h-9 px-3.5 rounded-chip text-[12.5px] font-bold text-white bg-brand hover:bg-brand-dark shadow-brand" @click="openNew()"><Icon name="plus" :size="14" />{{ L("New structure", "هيكل جديد", "Nouvelle structure") }}</button>
    </div>
      ...
      <table v-else class="w-full text-[12px]">
        ...
        <tbody>
          <tr v-for="r in rows" :key="r.name" class="border-t border-line-hair hover:bg-app-warm/40">
            <td class="px-4 py-2"><span class="font-semibold">{{ r.name }}</span><span class="block text-[10.5px] text-ink-muted">{{ r.currency }} · {{ r.payroll_frequency }} · {{ r.modified }}</span></td>
            ...
            <td class="px-2 py-2"><span class="text-[10px] font-semibold px-1.5 py-0.5 rounded" :class="r.docstatus === 0 ? 'bg-amber-50 text-amber-700' : r.is_active === 'Yes' ? 'bg-emerald-50 text-emerald-700' : 'bg-app-warm text-ink-muted'">{{ r.docstatus === 0 ? L("Draft", "مسودة", "Brouillon") : r.is_active === 'Yes' ? L("Active", "نشط", "Actif") : L("Inactive", "غير نشط", "Inactif") }}</span></td>
            <td class="px-3 py-2 text-end whitespace-nowrap">
              <button type="button" class="text-[10.5px] font-bold px-2 py-0.5 rounded border border-line-2 hover:bg-app-warm" @click="openEdit(r)">{{ r.docstatus === 0 ? L("Edit", "تعديل", "Modifier") : L("Copy as new", "نسخ كجديد", "Copier") }}</button>
              <button v-if="canWrite && r.docstatus === 1" type="button" class="ms-1 text-[10.5px] font-bold px-2 py-0.5 rounded border border-line-2 hover:bg-app-warm" :disabled="busy" @click="toggleActive(r)">{{ r.is_active === 'Yes' ? L("Deactivate", "تعطيل", "Désactiver") : L("Activate", "تفعيل", "Activer") }}</button>
            </td>
          </tr>
        </tbody>
      </table>
      <div class="px-4 py-2 border-t border-line-hair text-[10px] text-ink-muted">{{ L("A submitted structure cannot change in ERPNext: copy it as a new one with the new amounts, then re-assign the employee.", "الهيكل المرحّل مش بيتعدل في ERPNext: انسخه كهيكل جديد بالمبالغ الجديدة واربط الموظف عليه.", "Une structure soumise ne se modifie pas : copiez-la puis réassignez.") }}</div>
    </div>
      ...
      <div class="bg-white rounded-card shadow-pop w-full max-w-2xl max-h-[92vh] flex flex-col">
        ...
        <div class="flex-1 overflow-auto px-5 py-4 space-y-3">
          <div class="grid gap-2 sm:grid-cols-3">
            <div class="sm:col-span-3"><label class="block text-[11px] font-bold text-ink-3 mb-1">{{ L("Name", "الاسم", "Nom") }} *</label><input v-model.trim="ed.structure_name" :disabled="!!ed.name" class="h-9 w-full rounded-[9px] border border-line-2 px-2.5 text-[12.5px] bg-white disabled:bg-app-warm" /></div>
            <div><label class="block text-[11px] font-bold text-ink-3 mb-1">{{ L("Currency", "العملة", "Devise") }}</label><input v-model.trim="ed.currency" dir="ltr" class="h-9 w-full rounded-[9px] border border-line-2 px-2.5 text-[12.5px] bg-white" /></div>
            <div><label class="block text-[11px] font-bold text-ink-3 mb-1">{{ L("Frequency", "الدورية", "Fréquence") }}</label><select v-model="ed.payroll_frequency" class="h-9 w-full rounded-[9px] border border-line-2 px-2 text-[12.5px] bg-white"><option v-for="f in (opts.frequencies || ['Monthly'])" :key="f" :value="f">{{ f }}</option></select></div>
            <div><label class="block text-[11px] font-bold text-ink-3 mb-1">{{ L("Paid via", "طريقة الدفع", "Mode") }}</label><select v-model="ed.mode_of_payment" class="h-9 w-full rounded-[9px] border border-line-2 px-2 text-[12.5px] bg-white"><option value="">—</option><option v-for="m in (opts.modes || [])" :key="m" :value="m">{{ m }}</option></select></div>
          </div>
          <div v-for="grp in ['earnings','deductions']" :key="grp" class="border border-line rounded-[12px] overflow-hidden">
            ...
            <div class="px-3 py-1.5"><button type="button" class="text-[11.5px] font-semibold text-accent hover:text-accent-dark inline-flex items-center gap-1" @click="ed[grp].push({ salary_component: '', amount: '' })"><Icon name="plus" :size="12" />{{ L("Add line", "إضافة سطر", "Ajouter") }}</button></div>
          </div>

  ...
  <div class="space-y-3.5">
    ...
    <template v-else>
      <div class="bg-white rounded-card border border-line shadow-card px-4 py-3.5 flex items-center gap-3 flex-wrap">
        ...
        <div class="min-w-0">
          <div class="text-[16px] font-extrabold font-mono">{{ d.run.name }}</div>
          <div class="text-[12px] text-ink-muted">{{ d.run.start_date }} → {{ d.run.end_date }} · {{ d.run.payroll_frequency }}</div>
        </div>
        <span class="ms-auto text-[11px] font-bold px-2 py-1 rounded-chip" :class="d.run.status==='Posted' ? 'bg-emerald-50 text-emerald-700' : d.run.status==='Cancelled' ? 'bg-rose-50 text-rose-600' : 'bg-amber-50 text-amber-700'">{{ d.run.status }}</span>
      </div>
        ...
        <table class="w-full text-[12px]">
          ...
          <tbody>
            <tr v-for="s in d.slips" :key="s.name" class="border-t border-line-hair hover:bg-app-warm/40 cursor-pointer group" @click="openSlip(s.name)">
              <td class="px-4 py-2.5"><div class="font-semibold group-hover:text-accent-dark">{{ s.employee_name }}</div><div class="text-[11px] text-ink-muted font-mono">{{ s.name }}</div></td>

    ...
    <template v-else>
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div class="bg-white rounded-card border border-line shadow-card px-4 py-3">
          <div class="text-[11px] font-bold uppercase tracking-wider text-ink-muted flex items-center gap-1.5"><Icon name="layers" :size="13" color="#0f766e" />{{ L("Accountants","المحاسبون","Comptables") }}</div>
          <div class="text-[20px] font-extrabold mt-1 tnum">{{ loading ? "—" : (t.members || 0) }}</div>
          <div class="text-[11px] text-ink-muted mt-0.5">{{ L("posted in this period","سجّلوا في هذه الفترة","actifs sur la période") }}</div>
        </div>
        <div class="bg-white rounded-card border border-line shadow-card px-4 py-3">
          <div class="text-[11px] font-bold uppercase tracking-wider text-ink-muted flex items-center gap-1.5"><Icon name="list" :size="13" color="#0369a1" />{{ L("Documents","المستندات","Documents") }}</div>
          <div class="text-[20px] font-extrabold mt-1 tnum" style="color:#0369a1">{{ loading ? "—" : (t.submitted || 0).toLocaleString() }}</div>
          <div class="text-[11px] text-ink-muted mt-0.5">{{ L("submitted","معتمدة","validés") }} · {{ money(t.value) }} {{ ccy }}</div>
        </div>
        <div class="bg-white rounded-card border shadow-card px-4 py-3" :class="t.rework_pct >= 20 ? 'border-rose-200' : 'border-line'">
          <div class="text-[11px] font-bold uppercase tracking-wider flex items-center gap-1.5" :class="t.rework_pct >= 20 ? 'text-rose-600' : 'text-ink-muted'"><Icon name="alert" :size="13" :color="t.rework_pct >= 20 ? '#e11d48' : '#94a3b8'" />{{ L("Rework rate","معدل إعادة العمل","Taux reprise") }}</div>
          <div class="text-[20px] font-extrabold mt-1 tnum" :class="reworkColor(t.rework_pct)">{{ loading ? "—" : (t.rework_pct || 0) + "%" }}</div>
          <div class="text-[11px] text-ink-muted mt-0.5">{{ (t.cancelled || 0).toLocaleString() }} {{ L("cancelled","ملغاة","annulés") }}</div>
        </div>
        <div class="bg-white rounded-card border shadow-card px-4 py-3" :class="t.flagged ? 'border-amber-200' : 'border-line'">
          <div class="text-[11px] font-bold uppercase tracking-wider text-ink-muted flex items-center gap-1.5"><Icon name="alert" :size="13" :color="t.flagged ? '#b45309' : '#94a3b8'" />{{ L("Flagged","تحت الملاحظة","Signalés") }}</div>
          <div class="text-[20px] font-extrabold mt-1 tnum" :class="t.flagged ? 'text-amber-700' : 'text-ink'">{{ loading ? "—" : (t.flagged || 0) }}</div>
          <div class="text-[11px] text-ink-muted mt-0.5">{{ L("high rework · stuck drafts","إعادة عمل عالية · مسودّات عالقة","reprise élevée · brouillons") }}</div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-card border border-line shadow-card overflow-hidden">
        <div class="px-4 py-2.5 border-b border-line-hair flex items-center gap-2">
          ...
          <span class="text-[11px] text-ink-muted">{{ L("ranked by volume","مرتّب حسب الحجم","par volume") }}</span>
          <span v-if="loading" class="text-[11px] text-ink-muted">…</span>
        </div>
              ...
              <tr v-for="(m, i) in members" :key="m.user" class="border-t border-line-hair cursor-pointer" :class="m.flagged ? 'bg-rose-50/40 hover:bg-rose-50' : 'hover:bg-app-warm/40'" @click="openMember(m)">
                ...
                <td class="px-4 py-2.5">
                  <div class="flex items-center gap-2.5">
                    <span class="w-7 h-7 rounded-full grid place-items-center text-[11px] font-bold text-white shrink-0" :style="`background:${avatarColor(m.user)}`">{{ initials(m.name) }}</span>
                    <div class="min-w-0">
                      <div class="font-semibold flex items-center gap-1.5">
                        <span class="text-ink-muted text-[11px] tnum w-4">{{ i + 1 }}</span>{{ m.name }}
                        <span v-if="m.flagged" class="text-[11px] font-bold text-rose-600 bg-rose-100 rounded px-1.5 py-0.5">{{ L("review","مراجعة","à revoir") }}</span>
                        <span v-if="!m.enabled" class="text-[11px] font-semibold text-ink-muted bg-app-warm rounded px-1.5 py-0.5">{{ L("disabled","معطّل","désactivé") }}</span>
                      </div>
                      <div class="text-[11px] text-ink-muted truncate">{{ m.user }} · {{ L("last","آخر","dernier") }} {{ m.last_at }}</div>
                    </div>
                  </div>
                </td>
                <!-- posted -->
                <td class="px-3 py-2.5 text-end">
                  ...
                  <div class="tnum text-[11px] text-ink-muted">{{ m.created.toLocaleString() }} {{ L("created","أنشأ","créés") }}</div>
                </td>
                <!-- doctype mix -->
                <td class="px-3 py-2.5 hidden md:table-cell">
                  <div class="flex items-center gap-1 flex-wrap">
                    <span v-for="dc in doctypes" v-show="m.by[dc.code]" :key="dc.code" class="text-[11px] font-semibold rounded px-1.5 py-0.5 tnum" :style="`background:${mixTint(dc.code)};color:${mixInk(dc.code)}`">{{ dc.code }} {{ (m.by[dc.code] || 0).toLocaleString() }}</span>
                  </div>
                </td>
                ...
                <td class="px-3 py-2.5 text-end">
                  ...
                  <div class="tnum text-[11px] text-ink-muted">{{ m.cancelled.toLocaleString() }}</div>
                </td>
      ...
      <div class="bg-white rounded-card border border-line shadow-card overflow-hidden">
        <div class="px-4 py-2.5 border-b border-line-hair flex items-center gap-2 flex-wrap">
          ...
          <span class="text-[11px] text-ink-muted">{{ L("every time someone still needed ERPNext — and what they did there","كل مرة حد لسه احتاج ERPNext، وعمل فيه إيه","chaque fois que quelqu'un a eu besoin d'ERPNext") }}</span>
          <span class="ms-auto flex items-center gap-1.5 text-[11px]">
        ...
        <div v-else-if="!dp.passes?.length" class="px-4 py-8 text-center text-[12px] text-ink-muted">
          ...
          <span v-if="dp.locked_users?.length" class="block mt-1 text-[11px]">{{ L("Locked:","مقفول عليهم:","Verrouillés :") }} {{ dp.locked_users.map((u) => u.full_name).join(" · ") }}</span>
          <span v-else class="block mt-1 text-[11px]">{{ L("Nobody is locked yet — Settings → Users → Desk.","لسه محدش مقفول عليه: الإعدادات ← المستخدمون ← الـ Desk.","Personne n'est verrouillé : Paramètres → Utilisateurs.") }}</span>
        </div>

        <template v-else>
          <!-- gap ranking -->
          <div class="px-4 py-3 border-b border-line-hair">
            <div class="text-[11px] font-bold uppercase tracking-wider text-ink-muted mb-2">{{ L("What keeps pulling them back — build these next","إيه اللي بيرجّعهم للـ Desk: ده اللي يتبني الأول","Ce qui les ramène au Desk") }}</div>
            <div class="grid gap-1.5 sm:grid-cols-2 lg:grid-cols-3">
              <div v-for="g in dp.gaps.slice(0, 9)" :key="g.doctype" class="flex items-center gap-2 text-[12px]">
            ...
            <table class="w-full text-[12px]">
              ...
              <tbody>
                <tr v-for="p in dp.passes" :key="p.id" class="border-t border-line-hair align-top">
                  ...
                  <td class="px-3 py-2">
                    <span v-if="!p.touched.length" class="text-[11px] text-ink-muted">{{ L("nothing recorded","لا شيء مسجَّل","rien") }}</span>
                    <span v-for="t in p.touched" :key="t.doctype" class="inline-flex items-center gap-1 text-[11px] font-bold px-1.5 py-0.5 rounded-full me-1 mb-1" style="background:#fef3c7;color:#92400e">

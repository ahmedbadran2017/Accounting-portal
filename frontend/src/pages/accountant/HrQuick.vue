  ...
  <div class="grid gap-3.5 lg:grid-cols-2">
    ...
    <div class="bg-white rounded-card border border-line shadow-card overflow-hidden">
      <div class="px-4 py-2.5 border-b border-line-hair flex items-center gap-2 flex-wrap">
        ...
        <input type="date" v-model="ci.from" @change="loadCheckins" class="fld fld-xs" />
        <span class="text-[11px] text-ink-muted">→</span>
        <input type="date" v-model="ci.to" @change="loadCheckins" class="fld fld-xs" />
      ...
      <div v-if="canWrite" class="px-4 py-2.5 border-b border-line-hair bg-app-warm/30 grid gap-2 sm:grid-cols-[1fr_auto_auto_auto] items-end">
        ...
        <div><label class="block text-[11px] font-bold text-ink-3 mb-0.5">{{ L("Type", "النوع", "Type") }}</label>
          <select v-model="ciForm.log_type" class="fld fld-sm"><option value="IN">IN</option><option value="OUT">OUT</option></select></div>
        <div><label class="block text-[11px] font-bold text-ink-3 mb-0.5">{{ L("Time", "الوقت", "Heure") }}</label><input type="datetime-local" v-model="ciForm.time" class="fld fld-sm" /></div>
        <UiButton variant="create" size="sm" type="button" :disabled="busy || !ciForm.employee || !ciForm.time" @click="addCheckin">{{ L("Add", "إضافة", "Ajouter") }}</UiButton>
      </div>
    ...
    <div class="bg-white rounded-card border border-line shadow-card overflow-hidden">
      <div class="px-4 py-2.5 border-b border-line-hair flex items-center gap-2 flex-wrap">
        ...
        <UiButton variant="create" size="sm" icon="plus" class="ms-auto" v-if="canWrite" type="button" @click="advOpen = !advOpen"> {{ L("New advance", "سلفة جديدة", "Nouvelle avance") }}</UiButton>
      </div>
      <div v-if="advOpen" class="px-4 py-3 border-b border-line-hair bg-app-warm/30 grid gap-2 sm:grid-cols-2">
        ...
        <div><label class="block text-[11px] font-bold text-ink-3 mb-0.5">{{ L("Amount", "المبلغ", "Montant") }}</label><input type="number" step="any" min="0" v-model="advForm.amount" dir="ltr" class="fld fld-sm w-full tnum" /></div>
        <div><label class="block text-[11px] font-bold text-ink-3 mb-0.5">{{ L("Date", "التاريخ", "Date") }}</label><input type="date" v-model="advForm.posting_date" class="fld fld-sm w-full" /></div>
        <div class="sm:col-span-2"><label class="block text-[11px] font-bold text-ink-3 mb-0.5">{{ L("Purpose", "الغرض", "Objet") }}</label><input v-model.trim="advForm.purpose" class="fld fld-sm w-full" /></div>
        <div><label class="block text-[11px] font-bold text-ink-3 mb-0.5">{{ L("Paid via", "طريقة الدفع", "Mode") }}</label>
          <select v-model="advForm.mode_of_payment" class="fld fld-sm w-full"><option value="">—</option><option v-for="m in advOpts.modes" :key="m.value" :value="m.value">{{ m.label }}</option></select></div>
        <div><label class="block text-[11px] font-bold text-ink-3 mb-0.5">{{ L("Advance account", "حساب السلف", "Compte d'avance") }}</label>
          <select v-model="advForm.advance_account" class="fld fld-sm w-full"><option value="">{{ L("company default", "افتراضي الشركة", "défaut") }}</option><option v-for="a in advOpts.accounts" :key="a.value" :value="a.value">{{ a.label }}</option></select></div>
        ...
        <div class="sm:col-span-2 flex items-center gap-2">
          ...
          <UiButton variant="primary" size="sm" class="ms-auto" type="button" :disabled="busy || !advForm.employee || !advForm.amount || !advForm.purpose" @click="createAdvance">{{ busy ? "…" : L("Book advance", "تسجيل السلفة", "Enregistrer") }}</UiButton>
        </div>
      </div>

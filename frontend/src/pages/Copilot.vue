    ...
    <div class="hidden md:flex w-[340px] lg:w-[380px] flex-shrink-0 border-e border-line flex-col min-h-0" style="background:rgba(255,255,255,.55)">
      <div class="px-4 pt-[15px] pb-3 border-b border-line-hair">
        <div class="flex items-center gap-2">
          <span class="text-[14px] font-bold">{{ L("CFO control","مدير الحسابات","Contrôle CFO") }}</span>
          <span v-if="live" class="text-[11px] font-bold px-1.5 py-0.5 rounded-full text-success-dark bg-success-soft">{{ L("LIVE","مباشر","LIVE") }}</span>
        </div>
      ...
      <div v-if="view === 'findings'" class="flex-1 overflow-y-auto min-h-0 p-3 flex flex-col gap-2.5">
        <div class="flex items-center gap-1 flex-wrap -mt-0.5">
          <button v-for="cf in CATS" :key="cf.key" @click="catFilter = cf.key" class="text-[11px] font-bold px-2 py-0.5 rounded-full border transition" :class="catFilter === cf.key ? 'bg-ink text-white border-ink' : 'bg-white text-ink-3 border-line-2'">{{ cf.label() }}<span v-if="catCount(cf.key)" class="ms-1 opacity-70">{{ catCount(cf.key) }}</span></button>
          <button v-if="canHunt" @click="runHunt" :disabled="hunting" class="ms-auto text-[11px] font-bold px-2 py-0.5 rounded-full text-white inline-flex items-center gap-1 disabled:opacity-60" style="background:linear-gradient(135deg,#a21caf,#6d28d9)" :title="L('AI hunt — Super Admin only (uses AI credits)','الصيد الذكي — للسوبر أدمن فقط (يستهلك رصيد AI)','Chasse IA — Super Admin')">
            <Icon name="search" :size="10" />{{ hunting ? L("Hunting…","يبحث…","…") : L("AI hunt","صيد ذكي","Chasse IA") }}
          </button>
        </div>
        <div v-if="huntNote" class="text-[11px] text-ink-muted bg-app-warm rounded-chip px-2.5 py-1.5 -mt-1">{{ huntNote }}</div>
          ...
          <div class="flex items-start gap-2.5">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-1.5 flex-wrap">
                ...
                <span class="text-[11px] font-bold px-1.5 py-0.5 rounded-badge" :style="catTag(a.category)">{{ catLabel(a.category) }}</span>
                <span v-if="a.score" class="text-[11px] font-bold px-1.5 py-0.5 rounded-badge bg-app-warm text-ink-3 inline-flex items-center gap-0.5" :title="L('Priority score — magnitude vs materiality × severity','أولوية — الحجم مقابل الأهمية × الخطورة','Priorité')"><Icon name="trend" :size="9" />{{ a.score }}</span>
                <span v-if="assigned[a.id]" class="text-[11px] font-bold px-1.5 py-0.5 rounded-badge bg-success-soft text-success-dark inline-flex items-center gap-0.5"><Icon name="check" :size="9" />{{ shortUser(assigned[a.id]) }}</span>
              </div>
      ...
      <div v-else class="flex-1 overflow-y-auto min-h-0 p-3 flex flex-col gap-2">
        <div v-for="t in board.tasks" :key="t.task" class="border border-line rounded-[12px] p-3 bg-white">
          <div class="flex items-start gap-2">
            <span class="w-6 h-6 rounded-full grid place-items-center text-[11px] font-bold text-white flex-shrink-0" :style="{ background: avatar(t.assigned_to) }">{{ initials(t.assigned_to) }}</span>
            <div class="flex-1 min-w-0">
    ...
    <div class="flex-1 flex flex-col min-w-0 min-h-0" style="background:linear-gradient(180deg,#faf9f8,#f6f4f2)">
        ...
        <div v-for="(m, i) in messages" :key="i" class="flex gap-[11px]" :class="m.role === 'user' ? 'justify-end' : 'items-start'">
          ...
          <div :class="m.role === 'user' ? 'max-w-[82%]' : 'max-w-[80%]'">
            ...
            <div v-if="m.proposal" class="mt-2.5 rounded-[12px] overflow-hidden" style="border:1px solid #e9d5ff;background:#faf7ff">
              <div class="flex items-center gap-1.5 px-3 py-2.5 border-b" style="border-color:#ede4fb"><Icon name="shield" :size="12" color="#7c3aed" /><span class="text-[12px] font-bold" style="color:#5b21b6">{{ m.proposal.title }}</span></div>
      ...
      <div class="px-4 sm:px-5 pt-3 pb-[18px] border-t border-line" style="background:rgba(255,255,255,.7)">
        <div class="flex gap-2 mb-2.5 flex-wrap">
          <button v-for="s in suggestions" :key="s" class="px-3 py-[7px] rounded-full bg-white border border-line-2 text-ink-2 text-[12px] font-medium hover:bg-app-warm" @click="quick(s)">{{ s }}</button>
        </div>

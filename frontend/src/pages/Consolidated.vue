  ...
  <div class="space-y-3.5">
    ...
    <div class="rounded-card p-5 text-white" style="background:linear-gradient(115deg,#1e1b3a,#3b2566 55%,#6d28d9)">
      <div class="flex items-center gap-2 mb-2.5 flex-wrap">
        ...
        <span v-if="isLive !== null" class="text-[11px] font-bold px-1.5 py-0.5 rounded-full" :style="isLive ? 'background:rgba(52,211,153,.22);color:#a7f3d0' : 'background:rgba(251,191,36,.22);color:#fde68a'">{{ isLive ? L("Live","مباشر","Live") : L("Load failed","فشل التحميل","Échec") }}</span>
        <span v-if="loadErr" class="text-[11px] text-amber-300/90 truncate max-w-[20rem]" :title="loadErr">{{ loadErr }}</span>
      </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3.5">
      <div v-for="k in kpis" :key="k.label" class="yo-card bg-white rounded-card border border-line p-4">
        ...
        <div class="text-[12px] text-ink-3 mt-0.5">{{ k.label }}</div>
        <div class="text-[11px] text-ink-muted mt-1.5">{{ k.sub }}</div>
      </div>
    </div>
    ...
    <div v-if="distorted" class="rounded-[12px] border border-amber-200 bg-amber-50 px-4 py-2.5 flex items-center gap-2.5">
      ...
      <span class="text-[12px] text-ink-2 flex-1">{{ L("Group net is inflated by Justyol Morocco's unrelieved inventory (the 685M stock / COGS break). Consolidated figures inherit that distortion until it's corrected.","صافي المجموعة متضخّم بسبب مخزون مغرب غير المُرحَّل لتكلفة المبيعات.","Le résultat groupe est gonflé par le stock non soldé du Maroc.") }}</span>
    ...
    <div class="bg-white rounded-card border border-line shadow-card overflow-hidden">
      <div class="px-4 py-2.5 border-b border-line-hair flex items-center gap-2"><Icon name="layers" :size="14" color="#7c3aed" /><span class="text-[12px] font-bold">{{ L("Per-entity contribution","مساهمة كل كيان","Contribution par entité") }}</span><span class="text-[11px] text-ink-muted">{{ L("translated to","محوَّل إلى","converti en") }} {{ d.base }}</span></div>
      <div class="overflow-x-auto">
        <table class="w-full text-[12px]">
          <thead><tr style="background:#fafaf9">
            <th class="px-4 py-2 text-start text-[11px] font-bold uppercase tracking-wider text-ink-muted">{{ L("Entity","الكيان","Entité") }}</th>
            <th class="px-4 py-2 text-end text-[11px] font-bold uppercase tracking-wider text-ink-muted">{{ L("FX","سعر","Taux") }}</th>
            <th class="px-4 py-2 text-end text-[11px] font-bold uppercase tracking-wider text-ink-muted">{{ L("Net (FY)","الصافي","Résultat") }}</th>
            <th class="px-4 py-2 text-end text-[11px] font-bold uppercase tracking-wider text-ink-muted">{{ L("Assets","الأصول","Actifs") }}</th>
            <th class="px-4 py-2 text-end text-[11px] font-bold uppercase tracking-wider text-ink-muted">{{ L("Cash","النقد","Trésorerie") }}</th>
          </tr></thead>
          <tbody>
            <tr v-for="(e, i) in d.rows" :key="i" class="border-t border-line-hair hover:bg-app-warm/50 cursor-pointer" @click="goEntity(e.company)">
              <td class="px-4 py-2.5"><span class="flex items-center gap-2"><span class="w-7 h-7 rounded-[8px] grid place-items-center text-white text-[11px] font-bold" :style="{ background: badge(e.company) }">{{ e.abbr || e.company.slice(0,2) }}</span><span><span class="block font-semibold">{{ e.company }}</span><span class="block text-[11px] text-ink-muted">{{ e.currency }}</span></span></span></td>
    ...
    <div class="bg-white rounded-card border border-line p-4 shadow-card">
      ...
      <div v-if="d.rate_warnings && d.rate_warnings.length" class="text-[11px] text-amber-700 mt-2">{{ L("No FX rate for","لا سعر صرف لـ","Pas de taux pour") }}: {{ d.rate_warnings.join(", ") }} — {{ L("translated at 1.0","محوَّل بسعر 1.0","converti à 1,0") }}</div>
    </div>
    ...
    <div class="bg-white rounded-card border border-line shadow-card overflow-hidden">
      <div class="px-4 py-3 border-b border-line-hair flex items-center gap-2 flex-wrap">
        ...
        <span class="text-[11px] text-ink-muted">{{ L("external revenue only · Morocco's corrected COGS (= the group's true cost) · everyone's OPEX","إيراد خارجي فقط · COGS المغرب المصحح (= تكلفة المجموعة) · مصاريف الجميع","revenu externe · COGS corrigé · OPEX de tous") }}</span>
        <div class="flex-1"></div>
        <button v-for="c in ['MAD','USD']" :key="c" class="h-[24px] px-2.5 rounded-[7px] text-[11px] font-bold border"
      ...
      <div v-else-if="g" class="overflow-x-auto">
        <table class="w-full text-[12px]">
          <thead><tr style="background:#fafaf9">
            ...
            <th class="px-3 py-2 text-end text-[11px] font-bold text-ink-muted">OPEX</th>
            <th class="px-4 py-2 text-end text-[11px] font-bold text-ink-muted">{{ L("Net","الصافي","Net") }}</th>
          </tr></thead>
          <tbody>
            <tr v-for="r in g.rows" :key="r.month" class="border-t border-line-hair">
              <td class="px-4 py-1.5 font-mono text-[11px]" dir="ltr">{{ r.month }}</td>
              ...
              <td class="px-3 py-1.5 text-center"><span class="text-[11px] font-bold px-1.5 py-0.5 rounded-full" :style="r.gm_pct >= 15 && r.gm_pct <= 75 ? 'background:#ecfdf5;color:#047857' : 'background:#fef2f2;color:#b91c1c'">{{ r.gm_pct }}%</span></td>
        ...
        <div class="px-4 py-2.5 border-t border-line-hair flex flex-wrap gap-x-5 gap-y-1 text-[11px] text-ink-muted">
        ...
        <div v-if="g.anomalies && g.anomalies.length" class="px-4 pb-2.5 text-[11px] text-amber-700">
    ...
    <div class="bg-white rounded-card border border-line shadow-card overflow-hidden">
      <div class="px-4 py-3 border-b border-line-hair flex items-center gap-2 flex-wrap">
        ...
        <span class="text-[11px] text-ink-muted">{{ L("the ledger column is what the balance sheet claims; the gap is value no warehouse holds","عمود الأستاذ العام هو اللي الميزانية بتدّعيه، والفرق قيمة مفيش مخزن شايلها","l'écart est une valeur qu'aucun entrepôt ne détient") }}</span>
      </div>
      ...
      <div v-else-if="inv" class="overflow-x-auto">
        <table class="w-full text-[12px]">
          <thead><tr style="background:#fafaf9">
            <th class="px-4 py-2 text-start text-[11px] font-bold text-ink-muted">{{ L("Entity","الكيان","Entité") }}</th>
            <th class="px-3 py-2 text-end text-[11px] font-bold text-ink-muted">{{ L("Units","قطعة","Unités") }}</th>
            <th class="px-3 py-2 text-end text-[11px] font-bold text-ink-muted">{{ L("In the warehouses","في المخازن","En entrepôt") }}</th>
            <th class="px-3 py-2 text-end text-[11px] font-bold text-ink-muted">{{ L("In the ledger","في الأستاذ العام","Au grand livre") }}</th>
            <th class="px-4 py-2 text-end text-[11px] font-bold text-ink-muted">{{ L("Gap","الفرق","Écart") }}</th>
          </tr></thead>
          <tbody>
            <tr v-for="r in inv.rows" :key="r.company" class="border-t border-line-hair">
              <td class="px-4 py-2">
                <span class="flex items-center gap-2">
                  <span class="w-6 h-6 rounded-[7px] grid place-items-center text-white text-[11px] font-bold" :style="{ background: badge(r.company) }">{{ r.company.slice(0,2) }}</span>
                  <span><span class="block font-semibold">{{ r.company }}</span><span class="block text-[11px] text-ink-muted">{{ r.role }} · {{ r.currency }}</span></span>
                </span>
              </td>
    ...
    <div class="bg-white rounded-card border border-line shadow-card overflow-hidden">
      <div class="px-4 py-3 border-b border-line-hair flex items-center gap-2">
        ...
        <span class="text-[11px] text-ink-muted">{{ L("each pair: seller-side docs vs what the buyer recorded — the gap is the work","كل زوج: فواتير البايع مقابل اللي المشتري سجله — الفرق هو الشغل","l'écart est le travail") }}</span>
      </div>
      <div v-if="m" class="p-4 space-y-3">
        <div v-for="p in m.pairs" :key="p.seller + p.buyer" class="border border-line rounded-[10px] overflow-hidden">
          <div class="px-3 py-2 flex items-center gap-2 flex-wrap" style="background:#fafaf9">
            <span class="text-[12px] font-bold">{{ p.seller }} → {{ p.buyer }}</span>
            <span class="text-[11px] tnum" dir="ltr">{{ L("seller","البايع","vendeur") }}: {{ p.seller_docs }} docs · {{ gm(p.seller_total) }} {{ p.seller_ccy }}</span>
            <span class="text-[11px] tnum" dir="ltr">{{ L("buyer recorded","المشتري سجل","acheteur") }}: {{ p.buyer_docs }} docs · {{ gm(p.buyer_total) }} {{ p.buyer_ccy }}</span>
            <span class="text-[11px] font-bold px-1.5 py-0.5 rounded-full" :style="p.buyer_docs >= p.seller_docs ? 'background:#ecfdf5;color:#047857' : 'background:#fef2f2;color:#b91c1c'">
        ...
        <div class="border border-line rounded-[10px] overflow-hidden">
          <div class="px-3 py-2 text-[12px] font-bold" style="background:#fafaf9">{{ L("IC balances awaiting settlement","أرصدة بينية في انتظار التسوية","Soldes IC") }}</div>
          <table class="w-full text-[11px]">
            <tbody>

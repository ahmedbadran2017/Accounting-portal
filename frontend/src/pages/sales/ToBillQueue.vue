    ...
    <div class="bg-white rounded-card border border-line overflow-hidden shadow-card">
      <div class="flex items-center gap-2.5 px-4 py-3 border-b border-line-hair flex-wrap">
        ...
        <div class="ms-auto relative">
          ...
          <input v-model.trim="st.search.value" :placeholder="L('Search DN / customer…','بحث…','Rechercher…')" class="fld fld-md fld-sunk w-44 sm:w-60" />
        </div>
      </div>
          ...
          <tbody>
            <tr v-for="r in st.rows.value" :key="r.name" class="border-t border-line-hair hover:bg-app-warm/50">
              ...
              <td class="px-4 py-2.5 text-end">
                <UiButton variant="secondary" size="xs" icon="receipt" :disabled="busy === r.name" @click.stop="bill(r)"> {{ busy === r.name ? L("…","…","…") : L("Bill","فوترة","Facturer") }}
                </UiButton>
              </td>
            </tr>
          </tbody>

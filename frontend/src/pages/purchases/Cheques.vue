<template>
  <div class="space-y-3.5">
    ...
    <div v-if="(sum.stale_n || 0) > 0" class="flex items-center gap-3 px-4 py-3 rounded-card border border-amber-200 bg-amber-50/70">
      ...
      <UiButton variant="primary" size="sm" icon="filter" class="ms-auto shrink-0" type="button" @click="setStatus('stale')"> {{ L("Review", "راجِع", "Revoir") }}
      </UiButton>
    </div>
    ...
    <div class="bg-white rounded-card border border-line overflow-hidden shadow-card">
      <div class="flex items-center gap-2.5 px-4 py-3 border-b border-line-hair flex-wrap">
        ...
        <div class="relative ms-auto">
          ...
          <input v-model.trim="srch" :placeholder="L('Cheque no / supplier…', 'رقم الشيك / المورّد…', 'N° chèque / fournisseur…')" class="fld fld-md fld-sunk w-44 sm:w-60" />
        </div>
      </div>

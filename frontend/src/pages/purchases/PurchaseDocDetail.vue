    ...
    <template v-else>
        ...
        <div class="flex items-start gap-3 flex-wrap">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <span class="text-[11px] font-bold px-2 py-0.5 rounded-full" :style="{ background: tint, color }">{{ DT_LABEL() }}</span>
              <span v-if="h.status" class="text-[11px] font-bold px-2 py-0.5 rounded-full" style="background:#f1efe8;color:#5f5e5a">{{ h.status }}</span>
            </div>
      ...
      <div class="grid lg:grid-cols-3 gap-3.5">
        ...
        <div class="lg:col-span-2 bg-white rounded-card border border-line shadow-card overflow-hidden">
          <div class="px-4 py-2.5 border-b border-line-hair flex items-center gap-2"><Icon name="list" :size="14" color="#0b5c4f" /><span class="text-[12px] font-bold">{{ L("Items","الأصناف","Articles") }}</span><span class="text-[11px] text-ink-muted">{{ d.items.length }}</span></div>
          <div class="overflow-x-auto">
            <table class="w-full text-[12px]">
              <thead><tr style="background:#fafaf9">
                ...
                <th class="px-4 py-2 text-end text-[11px] font-bold uppercase tracking-wider text-ink-muted">{{ L("Rate","السعر","PU") }}</th>

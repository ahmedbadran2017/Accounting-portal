      ...
      <div class="p-5">
        ...
        <div class="grid grid-cols-3 gap-2 mb-3">
          <div class="rounded-[10px] px-3 py-2" style="background:#fafaf9;border:1px solid #f0efed">
            <div class="text-[11px] text-ink-muted font-semibold uppercase tracking-wide">{{ L("Opening","رصيد افتتاحي","Ouverture") }}</div>
            <div class="text-[14px] font-bold tnum">{{ f2(s.opening) }}</div>
          </div>
          <div class="rounded-[10px] px-3 py-2" style="background:#fafaf9;border:1px solid #f0efed">
            <div class="text-[11px] text-ink-muted font-semibold uppercase tracking-wide">{{ L("Movement","الحركة","Mouvement") }}</div>
          ...
          <div class="rounded-[10px] px-3 py-2" style="background:#f0fdf4;border:1px solid #bbf7d0">
            <div class="text-[11px] text-ink-muted font-semibold uppercase tracking-wide">{{ L("Closing","رصيد ختامي","Clôture") }}</div>
            <div class="text-[14px] font-bold tnum text-success-dark">{{ f2(s.closing) }} <span class="text-[11px] text-ink-muted">{{ s.currency }}</span></div>
          </div>
        </div>
        ...
        <div v-else class="border border-line rounded-[10px] overflow-hidden overflow-x-auto">
          <table class="w-full text-[12px]">
            <thead><tr style="background:#fafaf9">
              <th class="px-3 py-2 text-start text-[11px] font-bold uppercase tracking-wider text-ink-muted">{{ L("Date","التاريخ","Date") }}</th>
              <th class="px-3 py-2 text-start text-[11px] font-bold uppercase tracking-wider text-ink-muted">{{ L("Voucher","المستند","Pièce") }}</th>
              ...
              <th class="px-3 py-2 text-end text-[11px] font-bold uppercase tracking-wider text-ink-muted">{{ L("Balance","الرصيد","Solde") }}</th>
            </tr></thead>
            <tbody>
              ...
              <tr v-for="(r, i) in (s.rows || [])" :key="i" class="border-t border-line-hair">
                ...
                <td class="px-3 py-1.5 font-mono text-[11px]">{{ r.doc }}<span v-if="r.type" class="text-ink-muted"> · {{ r.type }}</span></td>

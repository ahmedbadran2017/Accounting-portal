  ...
  <div v-if="s" class="space-y-3.5">
    <div class="bg-white rounded-card border border-line shadow-card p-4">
      <div class="flex items-center gap-3 flex-wrap">
        <span class="text-[16px] font-bold font-mono" dir="ltr">{{ s.pr }}</span>
        ...
        <span class="text-[12px]">{{ s.channel === "air" ? "🛫" : "🚢" }} {{ fmt0(s.kg) }}kg · {{ fmt0(s.qty) }} {{ L("units","قطعة","unités") }}
          <span v-if="s.freight.bill_kg && Math.abs(s.freight.bill_kg - s.kg) > 0.25 * Math.max(s.freight.bill_kg, s.kg)"
                class="text-[11px] font-bold px-1.5 py-0.5 rounded-full ms-1" style="background:#fffbeb;color:#b45309"
      ...
      <div class="max-h-[420px] overflow-y-auto">
        <table class="w-full text-[12px]">
          <thead><tr style="background:#fafaf9" class="sticky top-0 z-[1]">
            <th class="px-3 py-1.5 text-start text-[11px] font-bold text-ink-muted">{{ L("Item","الصنف","Article") }}</th>
            <th class="px-3 py-1.5 text-end text-[11px] font-bold text-ink-muted">{{ L("Qty","كمية","Qté") }}</th>
            <th class="px-3 py-1.5 text-end text-[11px] font-bold text-ink-muted">{{ L("Booked","المسجّل","Comptab.") }}</th>
            <th class="px-3 py-1.5 text-end text-[11px] font-bold text-ink-muted">{{ L("Suggested","المقترح","Suggéré") }}</th>
            <th class="px-3 py-1.5 text-end text-[11px] font-bold text-ink-muted w-[110px]">{{ L("Verified ✎","المعتمد ✎","Vérifié ✎") }}</th>
            <th class="px-3 py-1.5 text-end text-[11px] font-bold text-ink-muted">+ {{ L("freight","شحن","fret") }}</th>
            <th class="px-3 py-1.5 text-end text-[11px] font-bold text-ink-muted">{{ L("Full","الكاملة","Total") }}</th>
          </tr></thead>
          <tbody>
            <tr v-for="l in s.lines" :key="l.item_code" class="border-t border-line-hair" :style="l.fixed ? 'background:#f0fdf4' : ''">
              <td class="px-3 py-1.5">
                <span class="font-mono text-[11px]" dir="ltr">{{ l.item_code }}</span>
                <span v-if="l.fixed" class="text-[11px] font-bold ms-1" style="color:#047857">✓ {{ L("applied","مطبَّق","appliqué") }}</span>
                <!-- cross-shipment badge: the applied cost is weighted over ALL its shipments -->
                <span v-if="l.other_prs?.length" class="text-[11px] font-bold px-1.5 py-0.5 rounded-full ms-1 cursor-help"
                ...
                <div class="text-[11px] text-ink-muted truncate max-w-[260px]">{{ l.sku || l.item_name }}</div>
              </td>
    ...
    <div class="bg-white rounded-card border border-line shadow-card p-4">
      <div class="flex items-center gap-2 flex-wrap mb-2">
        ...
        <span class="text-[12px] tnum flex-1 text-end"><b>{{ fmt0(s.freight.landed) }}</b> <span class="text-[11px] text-ink-muted" dir="ltr">@{{ s.freight.rate_kg }}/kg</span></span>
      </div>
      ...
      <div :class="s.channel === 'air' ? 'mt-2.5 pt-2.5 border-t border-line-hair' : ''">
        ...
        <div class="flex gap-1.5 flex-wrap mb-1.5">
          <span v-for="b in attachedBills" :key="b.voucher" class="inline-flex items-center gap-1.5 text-[11px] border rounded-[8px] px-2 py-1" style="background:#f0fdf4;border-color:#bbf7d0">
            ...
            <span v-if="b.kg" class="text-[11px] tnum text-ink-muted" dir="ltr">{{ b.kg_source === 'auto' ? '🔒' : '' }}{{ fmt0(b.kg) }}kg @{{ b.implied_rate }}</span>
        ...
        <details class="text-[12px]">
          ...
          <div class="mt-2 border border-line rounded-[8px] max-h-[200px] overflow-y-auto">
            <div v-for="b in shownBills" :key="b.voucher" class="flex items-center gap-2 px-3 py-1.5 border-b border-line-hair last:border-0 hover:bg-app-warm">
              ...
              <span v-if="b.ref_match" class="text-[11px] font-bold px-1.5 py-0.5 rounded-full whitespace-nowrap" style="background:#ecfdf5;color:#047857"
              ...
              <button v-if="canWrite && !s.frozen" :disabled="busy" class="text-[11px] font-bold px-2 py-0.5 rounded-[6px] border border-line hover:bg-white disabled:opacity-40" @click="toggleBill(b, true)">{{ L("Attach","إرفاق","Joindre") }}</button>
            </div>
          </div>
        </details>
        <div class="text-[11px] text-ink-3 mt-1">{{ L("Missing bills? Enter them from Purchases → New bill on the freight accounts — they appear here.","فيه فواتير ناقصة؟ دخّلوها من المشتريات (فاتورة جديدة) على حسابات الشحن — هتظهر هنا.","Saisir les factures manquantes dans Achats.") }}</div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-card border border-line shadow-card p-4">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
        <div class="border border-line rounded-[10px] px-3 py-2"><div class="text-[11px] text-ink-muted">{{ L("Goods (verified)","البضاعة (المعتمد)","Marchandise") }}</div><div class="text-[14px] font-bold tnum">{{ fmt0(goodsValue) }}</div><div class="text-[11px] text-ink-3">{{ verifiedCount }}/{{ s.lines.length }} {{ L("lines","سطر","lignes") }}</div></div>
        <div class="border border-line rounded-[10px] px-3 py-2"><div class="text-[11px] text-ink-muted">{{ L("Freight","الشحن","Fret") }}</div><div class="text-[14px] font-bold tnum">{{ fmt0(s.freight.landed) }}</div></div>
        <div class="border border-line rounded-[10px] px-3 py-2"><div class="text-[11px] text-ink-muted">{{ L("Shipment total","إجمالي الشحنة","Total") }}</div><div class="text-[14px] font-bold tnum">{{ fmt0(goodsValue + (s.freight.landed || 0)) }}</div></div>
        <div class="border rounded-[10px] px-3 py-2" :style="isComplete ? 'border-color:#a7f3d0;background:#ecfdf5' : 'border-color:#fde68a;background:#fffbeb'"><div class="text-[11px] text-ink-muted">{{ L("Avg full cost / unit","متوسط التكلفة الكاملة/وحدة","Coût moyen/unité") }}</div><div class="text-[14px] font-bold tnum">{{ s.qty > 0 ? ((goodsValue + (s.freight.landed || 0)) / s.qty).toFixed(2) : "—" }}</div><div class="text-[11px] font-bold" :style="isComplete ? 'color:#047857' : 'color:#b45309'">{{ isComplete ? "✅ " + L("complete","مكتملة","complète") : L("finish ① + ②","كمّلوا ① و②","finir ① + ②") }}</div></div>
      </div>
    </div>
      ...
      <div v-if="subPrev" class="mt-2.5 border-t border-line-hair pt-2 text-[11px] space-y-1">
        <template v-if="subPrev.dry_run">
          ...
          <div v-for="r in waitingRows.slice(0, 5)" :key="r.item_code" class="text-ink-muted">
            <span class="font-mono text-[11px]" dir="ltr">{{ r.item_code }}</span> →
            {{ r.status === "waiting_freight" ? L("freight of","شحن","fret de") : L("verify in","تحقق في","vérif. dans") }}
            <span class="font-mono text-[11px]" dir="ltr">{{ (r.prs || []).join(", ") }}</span>
          </div>

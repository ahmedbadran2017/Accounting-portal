    ...
    <div class="bg-white rounded-[18px] shadow-cardHover w-full max-w-md my-8 overflow-hidden">
      ...
      <template v-else>
        <div class="p-5 space-y-3.5">
          <label class="block"><span class="text-[11px] font-semibold text-ink-3">{{ L("From account","من حساب","Depuis") }}</span>
            <select v-model="fromAccount" class="mt-1 w-full border border-line-2 rounded-chip px-3 py-2 text-[12px] focus:outline-none focus:border-accent/40 cursor-pointer">
              <option value="">—</option>
              <option v-for="a in accounts" :key="a.name" :value="a.name">{{ label(a) }}</option>
            </select>
          </label>
          <div class="flex justify-center -my-1"><Icon name="arrow" :size="16" color="#9a8f86" class="rotate-90" /></div>
          <label class="block"><span class="text-[11px] font-semibold text-ink-3">{{ L("To account","إلى حساب","Vers") }}</span>
            <select v-model="toAccount" class="mt-1 w-full border border-line-2 rounded-chip px-3 py-2 text-[12px] focus:outline-none focus:border-accent/40 cursor-pointer">
              <option value="">—</option>
              <option v-for="a in accounts" :key="a.name" :value="a.name" :disabled="a.name===fromAccount">{{ label(a) }}</option>
            </select>
          </label>
        ...
        <div class="flex items-center justify-end gap-2 px-5 py-3.5 border-t border-line bg-app-warm/40">
          ...
          <button class="px-4 py-2 rounded-chip text-[12px] font-bold text-white bg-brand hover:bg-brand-dark shadow-brand disabled:opacity-50" :disabled="!canSubmit || posting" @click="submit">{{ posting ? "…" : L("Transfer","حوّل","Transférer") }}</button>
        </div>
      </template>
    </div>
  </div>
...
onMounted(async () => {
  try {
    ...
    if (p) {
      if (p.from_account) fromAccount.value = p.from_account;
      if (p.to_account) toAccount.value = p.to_account;

    ...
    <template v-else-if="d">
      ...
      <div class="bg-white border border-line rounded-[14px] shadow-card px-5 py-4 flex items-center gap-4 flex-wrap">
        ...
        <div class="min-w-0 flex-1">
          <div class="text-[16px] font-bold truncate">{{ modelName }}</div>
        ...
        <div class="text-end">
          <div class="text-[11px] text-ink-muted font-semibold">{{ L("Suggested (family evidence)","المقترح (دليل العائلة)","Suggéré") }}</div>
          ...
          <div v-if="d.model.basis_qty" class="text-[11px] text-ink-3 tnum">{{ L("basis","أساس","base") }} {{ d.model.basis_qty }}u</div>
        </div>
      </div>
      ...
      <div class="bg-white border border-line rounded-[14px] shadow-card overflow-hidden">
        ...
        <div class="p-4 space-y-3">
          <table v-if="d.evidence.length" class="w-full text-[12px]">
            <thead><tr style="background:#fafaf9">
              <th class="px-3 py-1.5 text-start text-[11px] font-bold text-ink-muted">{{ L("Document","المستند","Document") }}</th>
              <th class="px-3 py-1.5 text-start text-[11px] font-bold text-ink-muted">{{ L("Supplier","المورّد","Fournisseur") }}</th>
            ...
            <tbody>
              <tr v-for="e in d.evidence" :key="e.doc + (e.via || '')" class="border-t border-line-hair">
                <td class="px-3 py-1.5 font-mono text-[11px]" dir="ltr">{{ e.doc }}<span class="text-ink-muted font-sans"> · {{ e.dt }}</span><span v-if="e.via" class="font-sans text-[11px] text-violet-700"> 👪 {{ e.via }}</span></td>
      ...
      <div v-if="d.receipts && d.receipts.length" class="bg-white border border-line rounded-[14px] shadow-card overflow-hidden">
        <div class="px-4 py-3 border-b border-line-hair flex items-center gap-2">
          ...
          <span v-if="!d.waiting_prs.length" class="text-[11px] font-bold px-1.5 py-0.5 rounded-full" style="background:#ecfdf5;color:#047857">✓ {{ L("complete","مكتمل","complet") }}</span>
          <span v-else class="text-[11px] font-bold px-1.5 py-0.5 rounded-full" style="background:#fffbeb;color:#b45309">⏳ {{ d.waiting_prs.length }} {{ L("missing freight","ناقصها شحن","sans fret") }}</span>
        </div>
        <div class="overflow-x-auto max-h-[220px] overflow-y-auto">
          <table class="w-full text-[12px]">
            <thead><tr style="background:#fafaf9" class="sticky top-0">
              ...
              <th class="px-3 py-1.5 text-center text-[11px] font-bold text-ink-muted">{{ L("Ch.","قناة","Can.") }}</th>
              <th class="px-3 py-1.5 text-end text-[11px] font-bold text-ink-muted">{{ L("PR qty","كمية","Qté") }}</th>
              <th class="px-3 py-1.5 text-start text-[11px] font-bold text-ink-muted">{{ L("Freight","الشحن","Fret") }}</th>
              <th class="px-3 py-1.5 text-start text-[11px] font-bold text-ink-muted">{{ L("Variants in it","الأصناف فيها","Variantes") }}</th>
            </tr></thead>
            <tbody>
              <tr v-for="r in d.receipts" :key="r.pr" class="border-t border-line-hair">
                ...
                <td class="px-3 py-1.5 text-center whitespace-nowrap">
                  <button v-if="canWrite && !d.frozen" class="text-[12px]"
                          ...
                          @click="flipChannel(r)">{{ r.channel === "air" ? "🛫" : "🚢" }}<span v-if="!r.channel_confirmed" class="text-[11px] text-amber-600 font-bold">?</span></button>
                  <button v-if="canWrite && !d.frozen && !r.channel_confirmed" class="ms-0.5 text-[11px] font-bold text-emerald-700 hover:underline"
                          :title="L('confirm as-is','تأكيد زي ما هي','confirmer')" @click="confirmChannel(r)">✓</button>
                  <template v-if="!canWrite || d.frozen">{{ r.channel === "air" ? "🛫" : "🚢" }}<span v-if="!r.channel_confirmed" class="text-[11px] text-amber-600 font-bold">?</span></template>
                </td>
                <td class="px-3 py-1.5 text-end tnum">{{ fmtNum(r.pr_qty) }}</td>
                <td class="px-3 py-1.5 whitespace-nowrap">
                  <template v-if="['bills','rate'].includes(r.source)">
                    <span class="text-[11px] font-bold px-1.5 py-0.5 rounded-full" style="background:#ecfdf5;color:#047857">{{ r.source === 'bills' ? L("bills","فواتير","factures") : L("rate ✓","سعر ✓","taux ✓") }}</span>
                    <span class="text-[11px] text-ink-muted" dir="ltr"> @{{ r.rate_kg }}/kg</span>
                  </template>
                ...
                <td class="px-3 py-1.5 text-[11px] text-ink-muted truncate max-w-[180px]">{{ r.members.join("، ") }}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div v-else-if="d.receipts && !d.receipts.length" class="rounded-[12px] border px-4 py-2.5 space-y-2" style="background:#fffbeb;border-color:#fde68a">
        <div class="flex items-center gap-2">
          <span class="text-[13px]">🚢❓</span>
          <span class="text-[12px]" style="color:#b45309">{{ L("IMPORTED but NO import receipts on record — stock arrived via manual entries, so freight can't distribute automatically. Estimate it here: real weight × the era's contract tariff, and it folds into the verified cost with an audit note.","مستورد لكن مفيش استلامات استيراد مسجلة — دخل باستلامات يدوية فالشحن مش بيتوزع تلقائيًا. قدّره هنا: الوزن الحقيقي × تعريفة العقد لفترته، وهيتجمع على التكلفة المعتمدة بملاحظة توثيق.","Importé sans réceptions — estimer le fret ici.") }}</span>
        </div>
        <div v-if="canWrite" class="flex items-center gap-2 flex-wrap">
          ...
          <span class="inline-flex gap-1">
            <button v-for="c in [[100,'جوي 25'],[110,'جوي 26'],[126,'جوي الآن'],[23.6,'بحري']]" :key="c[0]"
                    class="h-[22px] px-1.5 rounded-[6px] text-[11px] font-bold border"
      ...
      <div v-if="canWrite && suspectWeights.length && d.receipts && d.receipts.length" class="rounded-[12px] border px-4 py-2.5 flex items-center gap-2 flex-wrap" style="background:#fffbeb;border-color:#fde68a">
        <span class="text-[12px] font-bold">⚖️ {{ L("Family weight","وزن العيلة","Poids famille") }}</span>
      ...
      <div class="bg-white border border-line rounded-[14px] shadow-card overflow-hidden">
        <div class="px-4 py-3 border-b border-line-hair flex items-center gap-2">
          ...
          <span v-if="d.waiting_prs.length" class="text-[11px] font-bold px-2 py-0.5 rounded-full" style="background:#fffbeb;color:#b45309">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-[12px]">
            <thead><tr style="background:#fafaf9">
              <th class="px-3 py-2 text-center text-[11px] font-bold text-ink-muted">{{ L("Apply","تطبيق","OK") }}</th>
              <th class="px-4 py-2 text-start text-[11px] font-bold text-ink-muted">{{ L("Variant","الصنف","Variante") }}</th>
              <th class="px-3 py-2 text-end text-[11px] font-bold text-ink-muted">{{ L("Qty","كمية","Qté") }}</th>
              <th class="px-3 py-2 text-end text-[11px] font-bold text-ink-muted">{{ L("Book","الدفاتر","Livre") }}</th>
              <th class="px-3 py-2 text-end text-[11px] font-bold text-ink-muted">kg</th>
              <th class="px-3 py-2 text-end text-[11px] font-bold text-ink-muted">{{ L("Landed","الشحن","Fret") }}</th>
              <th class="px-3 py-2 text-end text-[11px] font-bold text-ink-muted">{{ L("New → Δ","الجديد → Δ","Nouveau") }}</th>
              <th class="px-3 py-2 text-center text-[11px] font-bold text-ink-muted">{{ L("Status","الحالة","Statut") }}</th>
              <th class="px-3 py-2 text-center text-[11px] font-bold text-ink-muted"></th>
            </tr></thead>
            <tbody>
              <tr v-for="v in d.variants" :key="v.item_code" class="border-t border-line-hair" :class="results[v.item_code] === 'ok' ? 'bg-emerald-50/40' : ''">
                ...
                <td class="px-4 py-1.5">
                  <span class="inline-flex items-center gap-2">
                    ...
                    <span class="min-w-0"><span class="font-semibold">{{ v.sku || v.item_code }}</span><span class="text-[11px] text-ink-muted"> · {{ v.item_name }}</span></span>
                  </span>
                </td>
                ...
                <td class="px-3 py-1.5 text-end tnum whitespace-nowrap" dir="ltr">
                  <template v-if="rate > 0 && !v.fixed">
                    ...
                    <span class="text-[11px]" :style="{ color: (rate + v.landed_unit - v.book_rate) > 0 ? '#b45309' : '#047857' }">
                      {{ (rate + v.landed_unit - v.book_rate) > 0 ? "▲" : "▼" }}{{ fmtNum(Math.abs(rate + v.landed_unit - v.book_rate), 2) }}</span>
                  </template>
                  <span v-else class="text-ink-3">—</span>
                </td>
                <td class="px-3 py-1.5 text-center">
                  <span v-if="v.fixed" class="text-[11px] font-bold px-1.5 py-0.5 rounded-full" style="background:#ecfdf5;color:#047857">✓ {{ L("fixed","متظبط","corrigé") }}</span>
                  <span v-else-if="v.batch_tracked" class="text-[11px] font-bold px-1.5 py-0.5 rounded-full" style="background:#faf5ff;color:#7c3aed" :title="L('batch/serial tracked — manual path','بتتبع باتشات — مسار يدوي','suivi par lot')">🧬</span>
                  <span v-else-if="v.waiting.length" class="text-[11px] font-bold px-1.5 py-0.5 rounded-full" style="background:#fffbeb;color:#b45309" :title="v.waiting.join(', ')">⏳ {{ L("freight","شحن","fret") }}</span>
                  <span v-else class="text-[11px] font-bold px-1.5 py-0.5 rounded-full" style="background:#eff6ff;color:#2563eb">{{ L("ready","جاهز","prêt") }}</span>
                </td>
                <td class="px-3 py-1.5 text-center text-[12px]">
                  ...
                  <button class="ms-1 text-[11px] text-accent-dark hover:underline" :title="L('open the item page','افتح صفحة الصنف','ouvrir')"
      ...
      <div v-if="canWrite" class="bg-white border rounded-[14px] shadow-card px-4 py-3.5 flex items-center gap-3 flex-wrap" :style="canSubmit ? 'border-color:#a7f3d0' : 'border-color:#e7e5e4'">
        ...
        <template v-if="posting">
          ...
          <span class="text-[12px] tnum text-ink-muted">{{ progress }} / {{ targetCount }}</span>
        </template>
        <template v-else>

  ...
  <transition name="fade">
    <div v-if="t.selected.value.size" class="fixed bottom-5 inset-x-0 z-40 flex flex-col items-center justify-end px-4 pointer-events-none">
      ...
      <div v-if="asking" class="pointer-events-auto absolute bottom-[58px] w-[280px] bg-white rounded-[14px] border border-line shadow-pop p-3 space-y-2.5">
        ...
        <select v-if="asking.prompt.kind === 'user'" v-model="answer" class="h-9 w-full rounded-[9px] border border-line-2 px-2 text-[12.5px] bg-white">
        ...
        <input v-else v-model="answer" :placeholder="asking.prompt.placeholder || ''"
               class="h-9 w-full rounded-[9px] border border-line-2 px-2.5 text-[12.5px] bg-white focus:outline-none focus:border-accent/40" />
        <div class="flex justify-end gap-2">
          ...
          <button class="h-8 px-3.5 rounded-chip text-[12px] font-bold text-white bg-ink disabled:opacity-40" :disabled="!answer || busy" @click="confirmAsk">{{ asking.label }}</button>
        </div>
      </div>
    </div>
  </transition>
